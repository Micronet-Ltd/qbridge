...
namespace qbrdge_driver_classlib
{
	...
	public class RP1210DllCom
	{
		...
		public RP1210DllCom()
		{
			...
			try
			{
				_PacketCountMax = (int)Registry.GetValue(keyName, "PacketCountMax", 100);
				_DebugMode = (int)Registry.GetValue(keyName, "DebugMode", -1);
			}
		...
		public static void ParseUdpPacket(byte[] data, IPEndPoint iep)
		{
			...
			{
				if (_PacketCount > _PacketCountMax)
				{
					switch (_DebugMode)
					{
						case 0:
							//ISO DEBUG: Stop listening
							udpListener.Close();
							try
							{
								udpListenThread.Abort();
							...
							return;
						case 1:
							//ISO DEBUG: Simulate no response
							return;
						//break;
						case 2:
							//ISO DEBUG: Graceful End
							EndProgram();
							break;
						case 3:
							//ISO DEBUG: KILL KILL KILL
