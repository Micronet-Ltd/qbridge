...
/* readSRecFile */
/****************/
int readSRecFile(readSRecItem *srec, char *filename)
{
   int tmp;
   long idx;
   int retVal = 0;
   ...
   srec->len = srec->maxAddr - srec->minAddr;
   srec->buf = (unsigned char *) malloc(srec->len);
   if (srec->buf == NULL) {
      /* Out of memory */
      fprintf(stderr, "Out of memory!\n");
      retVal = -1;
   ...
   /* Now go back through the data, copying it to our output buffer */
   for (tmpptr = srec->buf, idx = 0;;) {
      tmp = scanLine(&item, in);
      idx++;
      if (tmp == 1) {
         /* End of file reached - do this here, so we can break */
      ...
      case 3:
         ...
         memcpy(tmpptr + (item.addr - srec->minAddr), item.data, item.count);
         break;
      default:
         fprintf(stderr, "Currently unsupported srec type %d!\n", item.type);
         retVal = -1;
   ...
   switch (verbose) {
   case 1:
      idx = sprintf(verboseStr, "%s", filename);
      verboseStr += idx;
      for (; idx < 20; idx++) {
         *(verboseStr++) = ' ';
      }
...
{
	fprintf(stdout, "Usage: %s <srec>\n", progname);
	fprintf(stdout, "       Fills in the length and crc in the header of the srec image\n");
}
...
int main(int argc, char *argv[])
{
   ...
   if (srec == NULL) {
	...
	retVal = readSRecFile(srec, argv[1]);
	if (retVal != 0) {
		/* Error has already printed */
		goto cleanExit;
	}

   /* Now check that the first one has a CRC header */
   romHdr = (crcROMHdrDefn *) srec->buf;
   if (romHdr->magic != crcROMMagicL) {
      fprintf(stderr, "Primary file does not have a CRC header!\n");
      retVal = -1;
      goto cleanExit;
   }

	/* Fill in length in header */
   romHdr->codeLen = srec->len;

   /* Generate the CRC */
   createCITTTable(CITTTable);
   romHdr->crc = calcCITT(CITTTable, srec->buf + sizeof(crcROMHdrDefn), srec->len - sizeof(crcROMHdrDefn));

   /* Now modify the srec file */
	if (writeSRecFile(srec, argv[1]) != 0) {
   ...
   switch (verbose) {
   case 1:
      fprintf(stdout, "Produced %s   size: %d bytes   CRC: 0x%04x\n", argv[1], romHdr->codeLen, romHdr->crc);
      /* Fall-through */
   default:
      break;
   }
...
#if 0
	if (fwrite(recs[idx].buf, 1, recs[idx].len, out) != recs[idx].len) {
